  ...
  while (true) {
    ...
    if (main.students === "Add a student") {
      ...
      if (allStudents === 0) {
        ...
        console.log(
          `${chalk.blue("Total Students : ")} ${chalk.blueBright(
            allStudents.length
          )}`
        );
        console.table([allStudents]);
      ...
      const searchStu = await inquirer.prompt([
        {
          ...
          type: "input",
          message:
            "Enter The Name of The Student You Are Looking For Or Part Of The Name",
        },
      ]);
      const filteredStu = allStudents.filter((student: any) =>
        student.includes(searchStu.search)
      );
      if (filteredStu.length === 0) {
        ...
        console.log(
          chalk.cyan(
            `The Student You Are Looking For Is ${chalk.bgCyan(filteredStu[0])}`
          )
        );
      }
      ...
      allStudents.find((student: any) => student.names === payFees.fees);
