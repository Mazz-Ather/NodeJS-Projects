...
let q1: any = await inquirer.prompt([
  {
    ...
    type: "list",
    message: chalk.blue(
      "1. Which of the following is a benefit of using TypeScript over JavaScript?"
    ),
    choices: [
      "a) Improved performance at runtime",
...
if (q1.q1 === "d) Static type checking") {
  ...
  console.log(
    chalk.red(`True answer is ${chalk.bgRed("d) Static type checking")}`)
  );
...
let q2 = await inquirer.prompt({
  ...
  type: "list",
  message: chalk.blue(
    "2. How do you declare a variable with a string type in TypeScript?"
  ),
  choices: [
...
if (q2.q2 === `b) let name:string = "John"`) {
  ...
  console.log(
    chalk.red(`True answer is ${chalk.bgRed(`b) let name:string = "John"`)}`)
  );
  console.log(`your score is ${chalk.red(score)}`);
}
...
let q3 = await inquirer.prompt({
  ...
  type: "list",
  message: chalk.blue(
    "3. What is the syntax for defining a function that takes two numbers as arguments and returns their sum in TypeScript?"
  ),
  choices: [
    `(a) function sum(x, y) { return x + y; }`,
...
if (q3.q3 === `d) All of the above`) {
  ...
  console.log(
    chalk.red(`True answer is ${chalk.bgRed(`d) All of the above`)}`)
  );
  console.log(`your score is ${chalk.red(score)}`);
}
let q4 = await inquirer.prompt({
  ...
  type: "list",
  message: chalk.blue(
    "4. Which of these is NOT a valid data type in TypeScript?"
  ),
  choices: [`a) boolean`, `b) object`, `c) float`, `d) enum`], //*c
});
